...
excerpt: 'このブログで計測した Core Web Vitals とコンバージョン率の関係を公開します。表示が遅いとコンバージョン率が下がるという通説を、自社の実データで確かめました。'
...
時系列で見ると、快適なLCP(緑)は安定してコンバージョンを得ているのに対し、要改善とストレスを感じる水準ではコンバージョンがない(CVR=0%)時間帯も多くあります。
...
ここでFIDのCVRだけ数値の大きさが気になったかもしれません。FID(First Input Idle)はユーザーの最初の操作に対する応答時間で、**ユーザーが操作を行わないことには計測されません。**少なくとも何らかのアクションを起こしたユーザーからしかサンプルを収集できないので、自ずとCVRも高くなるのです。
...
このブログではLCPが快適な場合(2.5秒未満)のCVRが13.47%でした。LCPを改善してもこれ以上スループットは上がらない、**LCPの改善によるコンバージョン率向上の限界**だと仮定できます。
...
広告による集客増は資金を投入し続ける必要があります。一方、**Web Vitals改善による燃費改善は一度実施すれば効果が持続**します(負荷上昇や大きな環境変化のない限り)。これはSEOが好まれる理由と同じで魅力的です。
...
LCPは表示速度を定量化する上での一面的な指標にすぎません。実際には『サイトの応答時間の短縮』が、LCPの改善や離脱減少・コンバージョン率向上につながっているだけです。
