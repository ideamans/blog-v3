...
excerpt: 'こんにちは！Rankletで表示しているランキング記事のサムネイルの画像はそのページのOGPの画像が標準で指定されています。ただ、OGP画像は800px以上（1200px推奨）で設定されていることが多いため、サムネイルとしての表示サイズに対して、実際の画像データのサイズが大きな画像となります。'
