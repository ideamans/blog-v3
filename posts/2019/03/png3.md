...
excerpt: '画質の劣化がなく背景の透過も可能なPNGは、キレイな分ファイルサイズも大きくなりがちです。よくJpegに対して3倍の重さと言われていますが、写真のような色数の多い画像はもっと重くなります。'
...
画質の劣化がなく背景の透過も可能なPNGは、キレイな分ファイルサイズも大きくなりがちです。
...
例えば次の画像は [pngquant](https://pngquant.org/) のページからの引用です。片方はPNG32、もう片方はPNG8で、ファイルサイズの差は約4倍です。どうでしょう、一瞬で画質の優劣を判別できますか？
...
他のツール(ImageMagickなど)では、パラメータを最適化しないと知覚的な画質が大きく低下するケースもあります。よく確認してから利用しましょう。
...
このチャンクは拡張性が高い。そのため、アプリケーションごとに独自のメタデータを埋め込むケースが多く見られます。これがファイルによっては大きなサイズを占めることがあります。
