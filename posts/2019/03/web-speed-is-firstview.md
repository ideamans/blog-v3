...
excerpt: 'Webの表示高速化は、多くの制作現場において難易度の高いミッションになりがちです。選択肢多すぎ問題ネットワーク・バックエンド・フロントエンドのどれから着手すべきか、それぞれ多くの手法のどれから着手するべきか、どのページから着手すべきか。'
...
はじめからサイト全体、ページ全体の軽量化を考えるより、そう考えるほうが気持ち的に楽ではないでしょうか？

実際にPageSpeed Insightsでは、ファーストビューの描画速度の改善で劇的にスコアの上がるケースが多く見られます。ファーストビューの表示の早さは、[FCP、FMP、Speed Indexなどの指標で計測](https://havelog.ayumusato.com/develop/performance/e744-performance_metrics.html)できます。
