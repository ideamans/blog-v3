...
そこで曖昧な本家レポートは置いて、フロントエンドエンジニア向けに本当の弱点を洗い出し、具体的な目標を示してスコアアップへ集中する「裏レポートツール」を作りました。
...
端的に言うと「世界における推定順位」を意味しており、スコア50が速い・遅いの境界となる人並みのページという評価(中央値)です。
...
「軽さには気を使っているが、スコアに納得が行かない...」というサイトの大半は(おそらく不慣れな)JavaScriptに原因があります。
