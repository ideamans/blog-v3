...
excerpt: 'Core Web Vitals で競合を調べる手順を紹介します。データポータルと Chrome UX Report を使えばプログラミングは不要で、他社の数値も見られます。'
...
`「Enter origin URL:」がレポートで変更されることを許可します`にチェックを付けます。`新しいレポートでレポート テンプレートを使用する`は最初からチェックが付いていると思います。これも外さず、最後に`接続`ボタンを押します。
