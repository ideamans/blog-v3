...
excerpt: '当たり前のような話ですが、念のため検証してみました。もちろん同じ画素数でも画像の内容(情報量≒乱雑さ)によってファイルサイズはバラバラになるので、あくまで同じ内容の画像、同じ品質パラメータという前提です。'
...
PNGの圧縮は連続パターンの長さで効率が変わるので、解像度の高い方ほど単位面積(画素数)あたりのファイルサイズが小さくなるのかも？
...
Webで画像によりユーザーに伝えたい情報量や思いの強さは概ね総画素数に代表されます。それが軽いほど、画像の利用効率の高いサイトという発想です。
