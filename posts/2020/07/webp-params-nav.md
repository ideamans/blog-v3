...
excerpt: 'cwebp コマンドの各パラメーターがWebP変換にどう効くかを、視覚的に比較できる「WebPパラメーターナビ」を公開しました。画質とファイルサイズを並べて確認できます。'
...
個人的に一番お気に入りのやりすぎ機能、**キャプチャ機能**です。マウスオーバーした際に右上へ**cキーでキャプチャ**と表示される領域は、文字通り`c`キーを押すだけで部分キャプチャできます。
